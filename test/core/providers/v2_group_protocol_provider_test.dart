...
void main() {
  group('V2GroupProtocolController', () {
    ...
    test('createGroupConversation stores group members and sends invites', () async {
      ...
      final localIdentity = await container.read(identityServiceProvider).bootstrap();
      ...
      expect(
        members.where((member) => member.role == 'admin').map((m) => m.peerId),
        contains(localIdentity.peerId),
      );
      expect(
        members.where((member) => member.role == 'invited').map((m) => m.peerId),
    ...
    test('incoming invite creates a pending group conversation for the local user', () async {
    ...
    test('sendGroupTextMessage sends to joined reachable members only', () async {
      final peersRepository = container.read(peersRepositoryProvider);
      final conversationsRepository = container.read(conversationsRepositoryProvider);
      final messagesRepository = container.read(messagesRepositoryProvider);
      final localIdentity = await container.read(identityServiceProvider).bootstrap();
      ...
      final conversation = await conversationsRepository.createGroupConversation(
